...
public class MapleQuestRequirement implements Serializable {
    ...
    public MapleQuestRequirement(MapleQuest quest, MapleQuestRequirementType type, ResultSet rse) throws SQLException {
        ...
        switch (type) {
            case pet, mbcard, mob, item, quest, skill, job -> {
            ...
            case partyQuest_S, dayByDay, normalAutoStart, subJobFlags, fieldEnter, pettamenessmin, npc, questComplete, pop, interval, mbmin, lvmax, lvmin -> {
    ...
    public boolean check(MapleCharacter c, Integer npcid) {
        switch (type) {
            ...
            case subJobFlags -> {
                // 1 for non-DB, 2 for DB...
                return c.getSubcategory() == (intStore / 2);
            }
            case craftMin, willMin, charismaMin, insightMin, charmMin, senseMin -> {
